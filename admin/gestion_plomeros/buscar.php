...
// Obtiene el término de búsqueda y la página actual
$search = isset($_GET['query']) ? mysqli_real_escape_string($conexion, $_GET['query']) : '';
...
$sql = "SELECT 
        ...
        WHERE 
            u.tipo_cuenta = 'work' 
            AND (u.nombre LIKE '%$search%' OR u.apellido LIKE '%$search%' OR u.correo LIKE '%$search%' OR u.telefono LIKE '%$search%' OR CONCAT(u.nombre, ' ', u.apellido) LIKE '%$search%')
        LIMIT $limit OFFSET $offset"; // Limita el número de registros y establece el offset
...
// Consulta para obtener el número total de registros que coinciden con el término de búsqueda
$total_sql = "SELECT COUNT(*) as total FROM usuarios u WHERE u.tipo_cuenta = 'work' AND (u.nombre LIKE '%$search%' OR u.apellido LIKE '%$search%' OR u.correo LIKE '%$search%' OR u.telefono LIKE '%$search%' OR CONCAT(u.nombre, ' ', u.apellido) LIKE '%$search%')";
...
<div class="flex justify-center mt-4">
    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
        <button class="mx-1 px-3 py-1 border rounded-md <?php echo $i == $page ? 'bg-primary text-white' : 'bg-white text-primary hover:bg-third hover:text-white'; ?>" onclick="fetchResults('<?php echo $search; ?>', <?php echo $i; ?>)">
            <?php echo $i; ?>
        </button>
    <?php endfor; ?>
</div>
